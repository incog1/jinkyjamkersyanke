...
void DrawMovementRecorder()
{
	for (int i = 0; i < 99999; i++)
	{
		...
		if (g_Render->WorldToScreen(Vector(m_RecordWaypoints[i].m_RecordedWaypoints.x, m_RecordWaypoints[i].m_RecordedWaypoints.y, m_RecordWaypoints[i].m_RecordedWaypoints.z), m_Point)) {
			...
			if (g_Render->WorldToScreen(Vector(m_RecordWaypoints[i - 1].m_RecordedWaypoints.x, m_RecordWaypoints[i - 1].m_RecordedWaypoints.y, m_RecordWaypoints[i - 1].m_RecordedWaypoints.z), m_Point1)) {
				//drawManager->drawLine(m_Point.x, m_Point.y, m_Point1.x, m_Point1.y, Color(255, 0, 0));
				g_Render->Outline((int)m_Point.x, (int)m_Point.y, 3, 3, col);
			}
		}
	}
}*/